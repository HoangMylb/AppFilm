...
const TimeSelect = ({ navigation }) => {
  const {getXuatPhim} = useContext(ThanhToanContext)
  ...
  const [showGio, setShowGio] = useState(false); // Thêm state để kiểm soát việc hiển thị FlatList của giờ

  const layXuatPhim = async () => {
    try {
      const response = await getXuatPhim(item._id, item1._id);
      if (response.success) {
       
        console.log('các xuất chiếu: ',response.message);
        const allGio = response.message.map((xuatPhimItem) => xuatPhimItem.gio).flat();
        const separatedGio = allGio.flatMap((gio) => gio.split(','));
        console.log('separatedGio: ', separatedGio);
        setngaySeat(allGio);
        const newData1 = separatedGio.map((gio, index) => ({
          id: index + 1,
          time: gio.trim() || "00:00",
          selected: false,
        }));
        const allNgay = response.message.map((xuatPhimItem) => xuatPhimItem.ngay);

        const newData = allNgay.map((ngay, index) => ({
          id: index + 1,
          time: ngay || "00:00",
          selected: false,
        }));
  ...
  const [data, setData] = useState([
    { id: 1, time: "3-3-2024", selected: false },
    { id: 2, time: "3-3-2024", selected: false },
    { id: 3, time: "3-3-2024", selected: false },
    { id: 4, time: "3-3-2024", selected: false },
  ]);
  useEffect(() => {
    layXuatPhim();
  }, []);
  ...
  const toggleSelection = (id) => {
    ...
    if (selectedDay) {
      ...
      console.log("Ngày đã chọn2222:", selectedDay.time);
      setSelectedNgay(selectedDay.time);

      // Hiển thị FlatList của giờ khi chọn một item ngày
      setShowGio(true);
    }
  };
  ...
  const toggleSelection1 = (id) => {
    ...
    if (selectedGio) {
      ...
      setSelectedGio(selectedGio.time);
    }
  };
      ...
      <TouchableOpacity onPress={() => toggleSelection(item.id)}>
        <View
          style={{
            width: 75,
            height: 75,
            backgroundColor: item.selected ? "#E38025" : "#3B3428",
            borderRadius: 80,
            marginLeft: 10,
            alignItems: "center",
  ...
  const nextTo = async () => {
    if (selectedNgay == "" || selectedGio == "") {
      ...
      navigateToSeatSelect(item1, item, idUser, selectedNgay, selectedNgay, ngaySeat, selectedGio, selectedNgay);
    }
  };
  ...
  return (
    <SafeAreaView style={{ backgroundColor: '#000000', flex: 1 }}>
      <ScrollView>
        ...
        {showGio && ( // Hiển thị FlatList của giờ nếu showGio là true
